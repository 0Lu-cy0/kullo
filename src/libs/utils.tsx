...
export const convertTreeData = (listData, withAnchor = false, id, name, parent, levelMin = 1) => {
  ...
  const treeDataConvert = convertChildrent(listRoot, listOther, withAnchor, id, name, parent)
  return treeDataConvert
}

const convertChildrent = (listRoot, listOther, withAnchor, id, name, parent) => {
  const newList = listRoot.map((root) => {
    const newItem = {
      ...root,
      title: withAnchor ? (
        <Link href={`#${root[id]}`} title={root[name]} style={{ minWidth: '200px' }} />
      ) : (
        root[name]
      ),
      label: root[name],
    ...
    if (listChild && listChild.length)
      return {
        ...newItem,
        children: convertChildrent(listChild, listOtherChild, withAnchor, id, name, parent),
      }
    return newItem
  })
  return newList
...
export function submitFormUsingCtrlS(form, handleSubmit, isForm, formRef = null) {
  document.onkeydown = (e) => {
    if (e.ctrlKey && e.keyCode === 83) {
      e.preventDefault()
...
export const checkSafari = () => /^((?!chrome|android).)*safari/i.test(navigator.userAgent)
...
export const checkSumFunc = (checkSum) => {
  ...
  const woringDayExcessive = checkSum?.workingDay_Thua?.map((item) => fdtfM2(item, 'm2e'))
  const workingDayLack = checkSum?.workingDay_Thieu?.map((item) => fdtfM2(item, 'm2e'))
  const status = {
    enough: 0,
    lack: 1,
    excessive: 2,
  ...
  switch (statusSum) {
    ...
    case status.excessive:
      result = {
        ...
        tooltip: (
          <>
            - Có {woringDayExcessive?.length} Ngày làm việc thừa thời lượng: <br />
    ...
    case status.lackExcessive:
      result = {
        ...
        tooltip: (
          <>
            - Có {workingDayLack?.length} Ngày làm việc thiếu thời lượng: <br />
            {workingDayLack.join(', ')} <br />- Có {woringDayExcessive?.length} Ngày làm việc thừa
            thời lượng: <br />
            {woringDayExcessive.join(', ')}
          </>
        ),
...
export const handleWorkingDay = (date, durationDate) => {
  ...
  switch (date) {
    case 1:
      ...
      break
    case 2:
      startDate = formatDate(moment(thisWeek).subtract(1, 'week').toDate(), DATE_FORMAT)
      endDate = formatDate(moment(), DATE_FORMAT)
      break
    case 3:
      ...
      break
    case 4:
      startDate = formatDate(moment(thisMonth).subtract(1, 'months').toDate(), DATE_FORMAT)
      endDate = formatDate(moment(), DATE_FORMAT)
      break
    case 6:
...
export const renderButtonCircle = (title, iconName, onClick, buttonCircle, btntype, fill, style) =>
  buttonCircle != null && (
    <ButtonCircle
...
export const getItemForFile = (nodeData) => {
  let iconName = ''
  const fileType = nodeData?.fileName.includes('.') ? nodeData?.fileName?.split('.') : ''
