...
export const disableBeforeDate = (current, endDate = dayjs().startOf('day')) => {
  return current && current >= endDate
}

export const disableAfterDate = (current, endDate = dayjs().startOf('day')) => {
  return current && current <= endDate
}
...
export function dtf2(dt, displayTime = false) {
  ...
  if (moment(dt).format('YYYY') === namnay) {
    if (moment(dt).format('WW') === tuannay && [homnay, ngaymai, ngaykia].indexOf(date) === -1)
      return moment(dt).format(`${time}[${getDayNameByDate(dt)}] [Tuần này (]DD/MM[)]`)

    if (+moment(dt).format('WW') === parseInt(tuannay) + 1)
      return moment(dt).format(`${time}[${getDayNameByDate(dt)}] [ Tuần sau (]DD/MM[)]`)
...
export function dtf4(dt) {
  ...
  const a = moment([moment().get('year'), moment().get('month'), moment().get('date')])

  const b = moment([moment(dt).get('year'), moment(dt).get('month'), moment(dt).get('date')])
...
export function dtf5(dt) {
  ...
  if (Math.abs(diffDay) >= 1 && Math.abs(diffDay) < 7) {
    ...
    if (+gio === 24) return `${dtf5Prefix(diffDay) + (Math.abs(diffDay) + 1)} ngày`
  ...
  if (diffMonth > 0) {
    if (dateDL > dateTD) {
      ...
      if (songay < 30) return `${dtf5Prefix(diffMonth) + Math.abs(diffMonth)} tháng ${songay} ngày`
...
export function dtf7(dt) {
  ...
  if (timeDetail.hours === 0)
    return `${timeDetail.minutes} phút trước ${moment(dt).format(`[(lúc ]H[h]mm['] [hôm nay)]`)}`

  if (diffDay === 0)
    return `${timeDetail.hours} giờ trước ${moment(dt).format(`[(lúc ]H[h]mm['] [hôm nay)]`)}`
